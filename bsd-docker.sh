...
clear
REUSE="No"
printf "\nDOCKER SETUP FOR ${BSD_COL}BITSEND (BSD)${NO_COL} RPC SERVER\n"
printf "\nSetup Config file"
printf "\n-----------------"
if [ -f "$CONFIG" ]
then
...
docker run -p ${DEFAULT_PORT}:${DEFAULT_PORT} -p ${RPC_PORT}:${RPC_PORT} -p ${TOR_PORT}:${TOR_PORT} --name ${CONTAINER_NAME} -e BSD_IP="${BSD_IP}" -e MN_KEY="${MN_KEY}" -e WEB="${WEB}" -e BOOTSTRAP="${BOOTSTRAP}" -v /home/bitsend:/home/bitsend:rw -d ${DOCKER_REPO}/bsd-masternode
...
if [ $? -ne 0 ];then
    ...
    printf "${GREEN}GREAT! Your ${BSD_COL}BitSend (BSD)${GREEN} Masternode Docker Container is running now! :)${NO_COL}\n"
